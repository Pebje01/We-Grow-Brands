...
export default function SuccessPopup({
  ...
  const displayTitle = title || (variant === 'error' ? t('errorTitle') : t('defaultTitle'))
  const displayMessage = message || (variant === 'error' ? t('errorMessage') : t('defaultMessage'))
  // Close on escape key
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        <h3 className={`text-center mb-3 ${isError ? 'text-red-600' : 'text-green-dark'}`}>{displayTitle}</h3>
        <p className="text-text-muted text-center">{displayMessage}</p>
